...
export const HISTORICO_GOVERNADORES: Record<string, MandatoHistorico[]> = {
  ...
  PE: [
    ...
    { nome: "Eduardo Campos", partido: "PSB", inicio: 2007, fim: 2014 },
    { nome: "Paulo Camara", partido: "PSB", inicio: 2015, fim: 2022 },
  ...
  SE: [
    ...
    { nome: "Marcelo Deda", partido: "PT", inicio: 2007, fim: 2013 },
...
export function getPresidente(ano: number): MandatoHistorico | null {
  return (
    HISTORICO_PRESIDENTES.find((p) => ano >= p.inicio && ano <= p.fim) ?? null
  );
}

export function getGovernador(
  uf: string,
  ano: number,
): MandatoHistorico | null {
